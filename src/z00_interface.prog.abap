...
CLASS lcl_rental IMPLEMENTATION.
  ...
  METHOD lif_partner~get_partner_info.
    rt_info = VALUE #( ( attribute =  `RENTAL` value = lines( mt_vehicles ) ) ).
  ENDMETHOD.
...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  METHOD lif_partner~get_partner_info.
    rt_info = VALUE #( ( attribute =  `CARRIER` value = lines( mt_planes ) ) ).
  ENDMETHOD.
...
DATA: go_airplane  TYPE REF TO lcl_airplane,
      ...
      gt_all_infos TYPE lif_partner=>tt_info.
...
* Create some vehicles.
  ...
  gt_partner = VALUE #( ( go_rental ) ( go_carrier ) ).

  LOOP AT gt_partner INTO DATA(go_partner).
    gt_all_infos = CORRESPONDING #( BASE ( gt_all_infos ) go_partner->get_partner_info( ) ).
  ENDLOOP.

  TRY.
      cl_s4d_output=>display_table( it_table = gt_all_infos ).
